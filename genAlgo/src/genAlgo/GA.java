...
public class GA {
	...
	private void doIt(int target) {
		...
		// Generate unique chromosome's in the pool
		...
		while(true) {
			...
			for(int x=pool.size()-1;x>=0;x-=2) {
				...
				if (n1.total == target && n1.isValid()) { System.out.println("1Generations: " + gen + "  Solution: " + n1.decodeChromo()); return; }
				if (n2.total == target && n2.isValid()) { System.out.println("2Generations: " + gen + "  Solution: " + n2.decodeChromo()); return; }
	...
	private static class Chomosone {
		...
		public Chomosone(int target) {
			...
			for(int y=0;y<chromoLen;y++) {
				...
				System.out.println("random binary integer: " + binString);
				// Fill to 4
			...
			System.out.println("random chromo binary integer - fill Length: " + chromo);
			// Score the new cromo
			scoreChromo(target);
		}
